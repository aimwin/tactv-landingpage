...
import circleImg from "../../assets/images/logo/tactv_logo.svg";
import BasicInformation from "./BasicInformation";
import Declaration from "./Declaration";
import TechnicalBankDetails from "./TechnicalBankDetails";
...
import { resetFormData, setCurrentPage } from "../../redux/slices/basicInformationSlice";

function LcoRegistration() {
  ...
  return (
    ...
    ${i + 1 < currentPage ? "text-white" : "text-gray-500"}`}
          >
            <div
              className={`w-8 h-8 xxl:w-10 xxl:h-10 flex items-center justify-center z-10 relative rounded-full font-semibold ${
                i + 1 < currentPage
                  ? "bg-green-400"
                  : currentPage === i + 1
                  ? "bg-sky-600"
            ...
            {i !== 0 && (
              <div
                className={`absolute w-full h-[3px] right-2/4 top-1/3 -translate-y-2/4 ${
                  i + 1 <= currentPage ? "bg-green-400" : "bg-slate-200"
                }`}
              />
            )}
          </div>
      ...
      <div className="font-poppins bg-[#914d1c26] flex items-center justify-between px-[4%] py-2">
        <div className="md:flex md:items-center">
          <p className="uppercase text-lg xxl:text-xl font-bold ">LCO Application form</p>
          <p className="md:ms-2 text-xs md:text-sm text-[#914D1C]">{formattedDate}</p>
        </div>
...
export default LcoRegistration;
