...
function Documents({ }: Props) {
  ...
  const [GSTUrlLoading, setGSTUrlLoading] = useState(false)
  const [GSTImgUrl, setGSTImgUrl] = useState('');
  const [GSTImgUrlError, setGSTImgUrlError] = useState('');
  ...
  useEffect(() => {
    if (formData) {
      ...
      setGSTImgUrl(formData.GSTImgUrl || '');
  ...
  const onSubmit = async (data: any) => {
    ...
    dispatch(updateFormData({ ...data, licenseImgUrl, GSTImgUrl, IDProofImgUrl, addressProofImgUrl, bankImgUrl, postalImgUrl }))
  ...
  const handleUploadGST = async (e: any) => {
    setGSTUrlLoading(true);
    if (e.target.files) {
      ...
      if (file.size > 2 * 1024 * 1024) {
        setGSTImgUrlError("upload file upto 2MB only");
        toast.error('File size exceeds 2MB.');
        setGSTUrlLoading(false);
        e.target.value = '';
        return;
      } else setGSTImgUrlError("");

             // Check file type
    if (!allowedFileTypes.includes(file.type)) {
      setGSTImgUrlError("Only JPG, PNG, or PDF files are allowed.");
      toast.error('Invalid file format. Please upload JPG, PNG, or PDF.');
      setGSTUrlLoading(false);
      e.target.value = ''; // Clear the file input
      return;
  ...
  useEffect(() => {
    ...
    if (allFormDetailsDosc) {
      ...
      setGSTImgUrl(allFormDetailsDosc.gst)
          ...
          <div className='border-[1px] border-[#DFDFDF] rounded-sm grid grid-cols-12 gap-5 mt-4 py-3 px-2'>
            <div className='col-span-12 md:col-span-6 lg:col-span-3 border-[1px] border-[#DFDFDF] rounded-sm w-full'>
              ...
              <div className='mx-4 bg-[#dfdfdfb0] rounded-md mt-3 py-2 mb-3'>
                <label htmlFor="uploadProfile" className='flex items-center mx-auto justify-center cursor-pointer'>
            ...
            <div className='col-span-12 md:col-span-6 lg:col-span-9'>
              <div className='grid grid-cols-12 gap-5'>
                ...
                <div className='col-span-12  lg:col-span-6 '>
                    ...
                    <div className='border-[1px] p-1 flex justify-between items-center border-[#DFDFDF] rounded-md'>
                      {GSTImgUrl ?
                        (
                              ...
                              {GSTImgUrl.endsWith('.pdf') ? 
                               ...
                               <FaEye className='w-5 h-5'
                                 onClick={() => {
                                   setDocumentItem({
                                       val: GSTImgUrl,
                                       key: GSTImgUrl?.endsWith('.pdf') ? 'document' : 'image'
                                   }),
                                       setDocumentModal(true);
                                   }}
                               />
                          ...
                          <>
                            <div className='ml-auto'>
                              <label className='bg-gray-100 border-[1px] p-2 cursor-pointer  rounded-md w-full flex items-center mx-auto justify-center text-sm' htmlFor="GST">
                                <MdOutlineFileUpload className='w-4 h-4 me-1' />
                                Upload File</label>
                              <input
                                id='GST'
                  ...
                  {GSTImgUrlError && <p className='text-red-400 text-xs mt-1 font-medium'>{GSTImgUrlError}</p>}
                </div>
